...
export class ConsumerAgentMessageTag extends TagAbstract {
     ...
     */
    public async getAll(agentId: string, refId?: number, chatId?: string): Promise<ConsumerAgentMessageCollection> {
     ...
     * @throws {ClientException}
     */
    public async submit(agentId: string, payload: AgentInput, refId?: number): Promise<AgentOutput> {
        const url = this.parser.url('/consumer/agent/$agent_id<[0-9]+|^~>/message', {
            'agent_id': agentId,
        });
